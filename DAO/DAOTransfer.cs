...
namespace DAO
{
    public class DAOTransfer
    {
        ...
        {
            string str = "UPDATE Player SET TeamID = '"+teamID+"',isSell=1 WHERE PlayerID = '" + id + "';\ndelete from Transfer where PlayerID = '"+id+"'";
            return (new DataProvider()).executeNonQuery(str);
        }
        ...
        {
            string str = "select * from Player where isMain = 'false' and isSell = 1 and TeamID = '"+acc.TeamID+"'";
            return (new DataProvider()).executeQuery(str);
        }
        ...
        public bool updateAfterSell(int id,int teamId)
        {
            string str = "UPDATE Player SET TeamID = '"+teamId+"',isSell= 0 WHERE PlayerID = '" + id + "'";
            return (new DataProvider()).executeNonQuery(str);
        }
